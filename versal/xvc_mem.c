...
static int open_port(void *client_data, XvcClient * c) {
    ...
    mem = (unsigned char*) mmap(0, xvc_mem->hub.size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_LOCKED, mem_fd, xvc_mem->hub.addr);
    if (mem == MAP_FAILED) {
    ...
    if (log_mode == LOG_MODE_VERBOSE)
        fprintf(stdout, "INFO: MMapped 0x%08lX to %p\n", (unsigned long) xvc_mem->hub.addr, (void *) xvc_mem->hub.buf);
...
static void mrd(
    ...
    if (1) { // Use single word reads if getting a "bus error" when using accesses unaligned to 64bits
        size_t i;
        
        for (i = 0; i < num_bytes; i += 4) {
            *(uint32_t *)(buf + i) = *(volatile uint32_t *)(xvc_mem->hub.buf + (addr - xvc_mem->hub.addr + i));
        }
    } else {
...
static void mwr(
    ...
    if (1) { // Use single 32bit word reads if getting a "bus error" when using accesses unaligned to 64bits
        size_t i;
...
static void display_banner() {
    fprintf(stdout, "\nDescription:\n");
    fprintf(stdout, "Xilinx xvc_mem v%s\n", XVCMEM_VERSION);
    fprintf(stdout, "Build date : %s-%s\n", date_stamp, time_stamp);
    fprintf(stdout, "Copyright 1986-2020 Xilinx, Inc. All Rights Reserved.\n\n");
}
